...
src/app/
  ...
  commlink/ tracking/ office-time/ notifications/ barcode/       ← timetracker domains
    commlink/{data,model,testing}/ = the dashboard read-model (eager slice, commlink-owned)
...
Centralized in `src/app/@shared/model/types.ts`. `IItemList<T>` requires `categories`/`mode` (grocery reads them unguarded; the tracking list carries empty defaults). `IDatastore` is the `DatabaseService` ↔ persisted-slice contract.
