...
Hash routing (`withHashLocation()`). Lazy pages: `commlink` (home/deck), `tracking`, `data/:listId`, `office-time`, `settings`, `barcode`, `notifications`, `soykaf` (**standby stub**), and the grocery routes `shopping/:listId`, `storage/:listId`, `tasks/:listId`, `products/:listId` (was `/database`; listId is `_products`), `list-settings`, plus `cash` and `trackplay/*`. Routes for the **lazy** contexts carry **`providers` (state + effects) + a `moduleHydrationResolver(load, loaded)`** that blocks activation until the scoped `loaded` fires: `tracking` + `data/:listId` (the tracking context — two slices `tracking`+`dialogs` co-registered); `notifications`; the grocery routes (co-register all three slices so cross-list reads never hit an unregistered sibling) + `tasks/:listId`; `cash`; the five `trackplay/*`; `settings` + `office-time` (the office-time context — two slices `settings`+`officeTime` co-registered); and `barcode` (its own single-slice context since sheriff-tighten §1 — was folded into the office-time context while the SIGIL badge lived in `officeTime`). `**` → `commlink`. Titles via `data.title` + `AppTitleStrategy`.

### State (NgRx)

Root (eager) `provideStore` slices are the **kernel only**: `router`, `dashboard` (commlink read-model), and the shared-kernel library slices `itemDialogs`/`listSettings`/`quickadd`. **Every bounded context is now lazy** (`feature/fully-lazy`, completing the deferred §7): `tracking`(+`dialogs`), `notifications`, `products`/`shopping`/`storage`/`tasks`, `cash`, `trackplay`, `barcode`, and `settings`/`officeTime` register per-route via `provideState` (`<domain>/data/provide-<m>-lazy.ts`) and hydrate in their route's `moduleHydrationResolver`, not eager. No global load action — each context owns a scoped `[X] load/loaded` lifecycle and its reducer hydrates on its own `loaded`; `main.ts`'s `provideAppInitializer` dispatches only the kernel loads (`listSettings` + `dashboard`).
...
- **List/dialog orchestrators are per-domain + lazy** (§2b). The old eager shell `grocery-list.effects.ts`/`item-dialogs.effects.ts`/`app.effects.ts` are **deleted**. The grocery multi-list engine + dialog orchestration live in `groceries/data/grocery-list/{grocery-list,grocery-item-dialogs}.effects.ts` (in `groceriesLazyProviders`, scoped to the three grocery lists); `tasks` has its **own switch-free copies** (`tasks/data/{tasks-list,tasks-item-dialogs}.effects.ts` in `tasksLazyProviders`, `@shared`-only → sealed). Each context's save + search + telemetry effects ride in its own `provide-<m>-lazy.ts` (`tracking/data/{tracking-search,tracking-save}.effects.ts`, `notifications/data/notifications-save.effects.ts`, etc.).

### Persistence (`src/app/@shared/util/database.service.ts`)

`@ionic/storage-angular`, DB name `np-commlink`, keys namespaced `npc-<slice>` (+ `npc-summary-<source>` for the persisted dashboard read-model). It is a **dumb per-key port**: `bootstrap()` runs the one eager boot read (the `npc-summary-*` docs, behind an init-once guard); each lazy context reads/writes only its own key via `load<T>(key)` / `save<K>(key, value)` in its own lazy load/save effect. `migrate()` (`migrations.ts`) is kept as a framework but is **empty** (VERSION `'1'`) — migrations no longer run at boot (§6). `itemDialogs`/`quickadd` are ephemeral and **not** persisted. Fresh-install only — no migration from the old `np-time-tracker` / kitchen-bot databases.
...
- Tokens live in **one** `:root` group: `src/theme/_shadowrun.scss` owns the `--sr-*` palette (the neon duo + the raw slate surfaces — the single source of truth), and `src/theme/variables.scss` *consumes* those into the Ionic `--ion-*` chrome. After the SCSS simplification (git log `scss-simplification Phase 1–6`) there are **no per-domain `--ion-color-<domain>` blocks** — every surface is uniform amber (`primary`) / teal (`secondary`); `TColor` (`@shared/types.ts`) is narrowed to Ionic's base palette. `@use`d once from `src/global.scss` (which also carries the Android safe-area map + `body.scanner-active` transparency).
- Deck signature pieces (HUD frame, corner brackets, status LEDs, brand row) are **Sass mixins** in the side-effect-free `src/theme/_deck.scss`; `_shadowrun.scss` wraps them as the global `.sr-panel`/`.sr-corners`/`.sr-led*`/`.sr-brand*` utility classes (emitted once). Consume via `var(--sr-*)` / the `.sr-*` classes, or `@use 'theme/deck'` + `@include` where a class can't reach (`:host`/pseudo-elements); **never `@use 'theme/shadowrun'` from a component** (it emits global CSS).
- `/commlink` deck is the reference expression; `/soykaf`, `/trackplay`, and the grocery/office-time pages all read the same shared classes. The 5 timetracker + 4 grocery programs render as HUD tiles (SOYKAF stays `standby`).
- **Second-theme seam:** `src/theme/palettes/` holds a documented, commented-out `.ion-palette-<name>` placeholder. Our default `:root` is already the dark deck, so a second theme overrides just the `--sr-*` token group under that class (reskinning Ionic chrome + deck together) — do **not** import Ionic's prebuilt `dark.class.css`.
