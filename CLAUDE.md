...
Hash routing (`withHashLocation()`). Lazy pages: `commlink` (home/deck), `tracking`, `data/:listId`, `office-time`, `settings`, `barcode`, `notifications`, `soykaf` (**standby stub**), and the grocery routes `shopping/:listId`, `storage/:listId`, `tasks/:listId`, `products/:listId` (was `/database`; listId is `_products`), `list-settings`, plus `cash` and `trackplay/*`. Routes for the **lazy** contexts carry **`providers` (state + effects) + a `moduleHydrationResolver(load, loaded)`** that blocks activation until the scoped `loaded` fires: the grocery routes (co-register all three slices so cross-list reads never hit an unregistered sibling) + `tasks/:listId`; `cash`; the five `trackplay/*`; and `settings` + `office-time` + `barcode` (the office-time context — `barcode` joins because the SIGIL badge lives in the `officeTime` slice; two slices co-registered). `**` → `commlink`. Titles via `data.title` + `AppTitleStrategy`.

### State (NgRx)

Root (eager) `provideStore` slices: `router`, `dashboard` (commlink read-model), `tracking`/`dialogs`/`notifications` (timetracker), `quickadd`/`listSettings`/`itemDialogs` (shared grocery kit). **Every bounded context is lazy except `tracking` + `notifications`** (`feature/lazy-modules`): `products`/`shopping`/`storage`/`tasks`, `cash`, `trackplay`, and `settings`/`officeTime` register per-route via `provideState` (`<domain>/data/provide-<m>-lazy.ts`) and hydrate in their route's `moduleHydrationResolver`, not eager. `tracking` + `notifications` stay eager because the background `TrackingNotificationsEffects` timer bridge reads/writes them off-route (a capability sink like `dashboard`; the deferred §7 decision — see `docs/lazy-modules-plan.md` §9). There is **no** global load action anymore — each context owns a scoped `[X] load/loaded` lifecycle and its reducer hydrates on its own `loaded`; `main.ts`'s `provideAppInitializer` dispatches only the eager loads (`tracking`/`notifications`/`listSettings` + `dashboard`).
