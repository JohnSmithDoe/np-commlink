 ...
 * `@Component({ selector })` and specs use them directly. A `data-testid`
 * on top of one is a second name for the same element — one more thing to
 * keep in sync, not more safety, and it splits "which elements do tests
 * depend on" across two mechanisms for no gain.
 *
 * Only the `app-` prefix is checked, which is exactly the set
 * `@angular-eslint/component-selector` already forces every component in
 * this repo to use. An `ion-*` element is not a component of ours and
 * legitimately needs an id, since its tag says something about iconography
 * rather than identity.
 *
 * Composed ids are `testid-is-static`'s job, so only the static attribute
 * is read here — an offending `[data-testid]` on an `app-*` element
 * reports once, there, rather than twice.
 * ───────────────────────────────────────────────────────────────── */
