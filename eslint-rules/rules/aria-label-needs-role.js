...
const NAME_PROHIBITING_ROLES = ['presentation', 'none', 'generic'];
  ...
  create(context) {
    ...
    return {
      Element(element) {
        ...
        const prohibiting = NAME_PROHIBITING_ROLES.includes(role ?? '');
        const roleless =
          ROLELESS_ELEMENTS.includes(element.name) &&
