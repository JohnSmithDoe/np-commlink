...
The other features → [features.md](./features.md) · theme label blocks →
[theming.md](./theming.md) (`DECK_CHROME_LABELS` is where a theme's voice lives) · why keys are declared, not composed → [i18n.md](./i18n.md).

`DECK_CATALOG` (`commlink/model/deck.catalog.ts`) is every navigable destination: the 13 deck
programs plus two menu-only rows (the deck's own home link, `/household/list-settings`). `CommlinkPage`
...
navigation, under what codename, at which grid position — `household` knows nothing about being
...
```ts
type DeckEntry = {
  id: DeckEntryId;        // stable — persisted config references it; never renamed
  module: AppModule;      // coarse: 'household'. NOT the per-aggregate telemetry `source`
  icon: string;
  route: string;
  titleKey: Marker;       // the *page's* title — the toolbar's, not the menu row's
  labels: Record<Theme, { nameKey: Marker; descKey: Marker }>;
  onDeck: boolean;        // false for menu-only entries
  source?: string;        // telemetry badge
  metric?: string;
  metricKey?: Marker;     // the badge's accessible name
...
`storage`), `module` per _domain_ (`household`) — which is why there must never be a second spelling
of `'household'` in the catalog.
...
single label: each entry declares `labels: Record<Theme, {nameKey, descKey}>` of `marker(...)`
literals that `resolveLabels` indexes by `ThemeService.theme`. Spelled out rather than composed,
because a composed key is invisible to `i18n:extract --clean` — and the `Record<Theme, …>`
...
"Einstellungen" twice — `/settings` and `/household/list-settings` share a title but have distinct
names.
