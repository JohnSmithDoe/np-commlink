...
describe('selectGameList — sort and filter', () => {
  ...
  it.each([
    ...
    ['updated', 'asc', ['b', 'a']],
    ['updated', 'desc', ['a', 'b']],
  ] as const)('sorts by %s %s', (sort, dir, expected) => {
    const state = withConfig(
      { games },
      { games: gamesConfig({ sort, dir, showEndedGames: true }) }
    );
...
describe('selectPlayerList — sort and filter', () => {
  ...
  it.each([
    ...
    const state = withConfig(
      { players },
      { players: playersConfig({ sort, dir }) }
    );
  ...
  it('treats a player who never played as least-recent', () => {
    const state = withConfig(
      ...
      { players: playersConfig({ sort: 'last', dir: 'asc' }) }
    );
...
describe('trackplay.selector — lookups by id', () => {
  ...
  it('has no rounds, scores or result for an unknown game', () => {
    ...
    expect(selectWinnerByGame('nope')(state)).toBeUndefined();
  });
});
