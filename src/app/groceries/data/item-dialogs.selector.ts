 ...
 * generic `selectEditState` / `selectEditItem`; each context casts them to its
 * own item type here, so the kernel never has to know grocery concepts
 * (product/shopping/storage). Once the item types move out of `@shared/types`
 * (the god-file split), these are the only casts that need to change.
 */
export const selectEditProductState = createSelector(
  selectEditState,
  (state): IEditProductState => state as unknown as IEditProductState
);
