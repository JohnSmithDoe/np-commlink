...
import { IListSettings, IQuickAddState } from '../model/list-settings.types';
import {
  IRecipe,
  IRecipeIngredient,
  IRecipesState,
...
export function mockRecipe(overrides: Partial<IRecipe> = {}): IRecipe {
  return {
    id: 'recipe-1',
    name: 'Pancakes',
    createdAt: TEST_TIMESTAMP,
...
export function mockRecipeIngredient(
  overrides: Partial<IRecipeIngredient> = {}
): IRecipeIngredient {
  return {
    id: 'ingredient-1',
    productId: 'product-1',
...
export function mockRecipesState(
  overrides: Partial<IRecipesState> = {}
): IRecipesState {
  return {
    id: RECIPES_LIST_ID,
    title: 'Recipes',
...
// (stateByListId, updateQuickAddState, filterBySearchQuery, …) take, and what a
// spec seeds under the `groceries` store key.
export function mockGroceriesState(
  overrides: Partial<IGroceriesState> = {}
): IGroceriesState {
  return {
