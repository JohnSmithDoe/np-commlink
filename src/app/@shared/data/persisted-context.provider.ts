...
import { MigrationStep } from '../util/db/versioned';
import {
  ...
  TSliceLifecycle,
  TTelemetrySpec,
} from './effects/persisted-slice.effects.factory';
...
// projection).
export type TPersistedContext<TState, TStored, S> = {
  key: string;
...
export function providePersistedContext<TState, TStored = TState, S = never>(
  ...
  return {
    ...
    resolve:
      hydrate === 'route'
        ? { [key]: moduleHydrationResolver(lifecycle.load, lifecycle.loaded) }
        : {},
  };
}
