...
**There is no second copy.** The workflow file publishes a tag and runs no gate, so this
suite is the whole of the checking and a gate that is not in `GATES=(` runs nowhere. It
splits what `pnpm run lint` chains behind one exit code — the plugin type-check, eslint
and **stylelint** — because one status across three tools cannot say which of them failed,
and hid stylelint entirely.
...
That order is fail-fast — cheapest and likeliest to fail first, so a typo does not cost
20s of Playwright before it surfaces.
...
| 12  | ESLint            | eslint                | **2–85s**  | the whole repo, not just `src/`; warm by default, `--cold` for the strict answer                  |
...
**~75–155s** — gate 12 is most of the spread, ~2s warm against ~30–85s cold, and swings
~50s with machine load even then. Cheap enough that "run the whole thing" is always the right answer; do
...
Gates 5, 9 and 12 are one command by hand — `pnpm run lint` = plugin `tsc` → eslint →
stylelint. Splitting them is the whole point of the runner: one exit code across three
tools is what hid stylelint.
...
- **The eslint cache is per-file; Sheriff is cross-file.** A violation that file A gains
  because file B changed can survive a warm run, and warm is the default — `--cold` purges
  the cache and is the strict answer. Note **`-rf`**: the cache is a *directory*, so `rm -f .eslintcache` fails with "is a
